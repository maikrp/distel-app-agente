...
/* ============================================================================
   GlobalSupervisorMenu Ver 1.4.7.jsx
...
export default function GlobalSupervisorMenu({ usuario }) {
  ...
  // Contexto de fecha para vistas "actual" y "anterior": 0 = hoy, 1 = ayer/último día (CR)
  const [offsetDiasCtx, setOffsetDiasCtx] = useState(0);

  // Fecha fijada cuando vista = "anterior" (último día laborable con data, L-S, excluye hoy)
  const [fechaFijadaCtx, setFechaFijadaCtx] = useState(null);

  // Métrica Liberty (control_registro)
  ...
  // === Fecha “hoy” Costa Rica (YYYY-MM-DD) ===
  const hoyISO = () => {
  ...
  const isoNDiasAtras = (n) => {
    const base = new Date().toLocaleDateString("en-CA", { timeZone: TZ }); // YYYY-MM-DD
  ...
  const parseISOasCRDate = (iso) => {
    ...
    // 12:00 UTC para evitar “off by one” al formatear en CR
    return new Date(Date.UTC(y, m - 1, d, 12, 0, 0));
  };

  // === Formato largo uniforme (📅 Viernes, 31 de oct de 2025) ===
  ...
  // Resumen global por fecha (hoy o fecha fijada) con efectividad y colores
  const cargarResumenGlobalGenerico = useCallback(
    async (offsetDias = 0, fechaForzada = null) => {
      setLoading(true);

      // Fecha referencia única para ambas tablas
  ...
  // Agentes de una región por fecha con efectividad y colores
  ...
  // Detalle de agente (hoy o fecha fijada si aplica) — incluye Motivos No Compra
  ...
  // === Última fecha laborable (L-S), excluye hoy ===
  const obtenerUltimaFechaLaborable = async () => {
    try {
      ...
      const esDomingoCR = (iso) => {
        const dCR = parseISOasCRDate(iso); // 12:00 UTC → evita desfases
        const day = dCR.getUTCDay(); // 0=domingo
        return day === 0;
      };
  ...
  // === MÉTRICA DE DESABASTO LIBERTY — Cálculo dinámico (hoy / último día) con guardado ===
  const cargarMetricaLiberty = useCallback(async () => {
    try {
      ...
      // 1) Total cargados (desabasto del día)
      ...
      // 3) total_excel del día en control_registro (más reciente <= fecha)
      ...
      // 4) cálculo final: (desabasto - efectivos) / total_excel
      ...
      // 5) Guardado automático si estamos en "actual"
  ...
  /* ============================== RENDERS ============================== */
  // Vista AdminTools (solo superadmin)
  if (vista === "adminTools") {
  ...
  if (vista === "agente" && detallesAgente && agenteSeleccionado && regionSeleccionada) {
    ...
    // Motivos de no compra (HOY/AYER según vista)
    const motivosMap = {};
  ...
  // Vista: resumenMotivos (por región, 7 días)
  if (vista === "resumenMotivos") {
    ...
    const pctEfectivosPais =
      totalAtendidosPais > 0 ? Math.round((totalEfectivosPais / totalAtendidosPais) * 100) : 0;
              ...
              {resumenMotivos.map(
                (r, idx) =>
                  (r.motivosPorcentaje || []).length > 0 && (
                    <div key={idx} className="bg-white shadow-lg rounded-3xl p-6 w-full max-w-6xl mx-auto mt-6 animate-fadeIn">
  ...
  // Vista: resumenMotivosRegion (detalle por agente, 7 días)
